...
interface SearchBarProps {
  onSearch?: (query: string, sources?: SearchSource[], bossMode?: boolean, filters?: any) => void;
  isLoading?: boolean;
  placeholder?: string;
}
...
const SearchBar: React.FC<SearchBarProps> = ({ 
  ...
  const [moq, setMoq] = React.useState<string>('');
  const [verifiedOnly, setVerifiedOnly] = React.useState(false);
  const [fiveStarOnly, setFiveStarOnly] = React.useState(false);
  const [excludeNoAddress, setExcludeNoAddress] = React.useState(false);
  const [excludeLowRating, setExcludeLowRating] = React.useState(false);

  const { performSearch, searchWithDebounce } = useSearch();
  ...
  React.useEffect(() => {
    ...
    const moqValue = moq ? parseInt(moq, 10) : undefined;
    const filters = {
      moq: moqValue,
      verifiedOnly,
      minRating: fiveStarOnly ? 5 : (excludeLowRating ? 3.1 : undefined),
      excludeNoAddress,
    };
  ...
  }, [searchQuery, selectedSources, bossModeEnabled, moq, verifiedOnly, fiveStarOnly, excludeNoAddress, excludeLowRating, canUseBossMode, searchWithDebounce]);
  ...
  const handleSearch = async (e: React.FormEvent<HTMLFormElement> | React.MouseEvent<HTMLButtonElement>) => {
    ...
    const moqValue = moq ? parseInt(moq, 10) : undefined;
    if (moq && (isNaN(moqValue!) || moqValue! < 1 || moqValue! > 99999)) {
      setError('MOQ must be between 1 and 99999');
      return;
    }
    ...
    if (onSearch) {
      onSearch(searchQuery, selectedSources, bossModeEnabled && canUseBossMode, filters);
      return;
    }
            ...
            <BlockStack gap="200">
              <InlineStack gap="200" align="center">
                ...
                {activeFiltersCount > 0 && (
                  <Badge tone="info" size="small">{`${activeFiltersCount} Active`}</Badge>
                )}
              </InlineStack>
              <InlineStack gap="400" wrap={true}>
                <Box minWidth="150px">
                ...
                <Box paddingBlockStart="400">
                  <InlineStack gap="400">
                    ...
                    <Checkbox
                      ...
                      onChange={setFiveStarOnly}
                      disabled={isLoading}
                    />
                    <Checkbox
                      label="Exclude No Address"
                      checked={excludeNoAddress}
                      onChange={setExcludeNoAddress}
                      disabled={isLoading}
                    />
                    <Checkbox
                      label="Exclude ≤3 Stars"
                      checked={excludeLowRating}
                      onChange={setExcludeLowRating}
                      disabled={isLoading}
                    />
                  </InlineStack>
                </Box>
