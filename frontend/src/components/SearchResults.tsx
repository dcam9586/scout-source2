    ...
    <div className="search-results" role="region" aria-label="Search results">
      ...
      <div className="search-results__footer">
        ...
        {onPageChange && totalCount > pageSize && (
          <div className="search-results__pagination" style={{ display: 'flex', gap: '8px', marginTop: '16px', justifyContent: 'center' }}>
            <button 
              disabled={currentPage <= 1}
              onClick={() => onPageChange(currentPage - 1)}
              style={{ padding: '8px 16px', borderRadius: '4px', border: '1px solid #ddd', background: currentPage <= 1 ? '#f5f5f5' : 'white', cursor: currentPage <= 1 ? 'not-allowed' : 'pointer' }}
            >
              Previous
            </button>
            ...
            <button 
              ...
              onClick={() => onPageChange(currentPage + 1)}
              style={{ padding: '8px 16px', borderRadius: '4px', border: '1px solid #ddd', background: currentPage >= Math.ceil(totalCount / pageSize) ? '#f5f5f5' : 'white', cursor: currentPage >= Math.ceil(totalCount / pageSize) ? 'not-allowed' : 'pointer' }}
            >
              Next
            </button>
          </div>
