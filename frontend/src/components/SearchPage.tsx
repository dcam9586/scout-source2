...
const SearchPage: React.FC<SearchPageProps> = () => {
  ...
  const isSearching = useAppStore((state) => state.isLoading);
  ...
  const totalCount = searchState?.totalResults || 0;
  const currentPage = searchState?.page || 1;
  const pageSize = searchState?.pageSize || 20;
  const activeFilters = searchState?.filters || {};
  const isBossMode = searchState?.bossMode || false;
  ...
  const handleSearch = useCallback(async (
    searchQuery: string, 
    sources: SearchSource[] = ['alibaba', 'made-in-china', 'cj-dropshipping'], 
    bossMode: boolean = false, 
    filters: any = {},
    page: number = 1
  ) => {
    // Set the current search query and sources IMMEDIATELY for the loading skeleton
  ...
  const handlePageChange = useCallback((newPage: number) => {
    handleSearch(query, searchedSources, isBossMode, activeFilters, newPage);
    // Scroll to top of results
    window.scrollTo({ top: 0, behavior: 'smooth' });
  }, [handleSearch, query, searchedSources, isBossMode, activeFilters]);
